...
function serveIndexHtml(req, res, indexPath, config) {
  ...
  const html = fs.readFileSync(indexPath, "utf8")
    .replace(/href="\/assets\/styles\.css\?v=wikist-core-20260711-50"/g, `href="${escapeHtml(assetUrl(config, "/assets/styles.css?v=wikist-core-20260711-50"))}"`)
    .replace(/src="\/assets\/app\.js\?v=wikist-core-20260711-50"/g, `src="${escapeHtml(assetUrl(config, "/assets/app.js?v=wikist-core-20260711-50"))}"`)
  ...
  return http.createServer(async (req, res) => {
    try {
      ...
      if (pathname.startsWith("/api/users/") && req.method === "GET") {
        ...
        sendJson(res, 200, { user: userPayloadWithHtml(profile, config) });
        return;
      }
      ...
      if (pageFavoriteMatch && req.method === "GET") {
        const slug = normalizeSlug(decodePathPart(pageFavoriteMatch[1]));
        if (!pages.getPage(slug)) {
          sendJson(res, 404, { error: "词条不存在。", slug });
          return;
        }
        sendJson(res, 200, { favorite: passport ? passport.pageFavoriteState(session?.user?.id, slug) : { pageSlug: slug, favorited: false, count: 0 } });
        return;
      }
