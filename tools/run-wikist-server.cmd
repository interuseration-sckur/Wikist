...
if /I "%~1"=="--restart" (
  ...
  powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$portText = $env:WIKIST_PORT; if ([string]::IsNullOrWhiteSpace($portText)) { $port = 8899 } else { $port = [int]$portText }; $listener = Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1; if ($listener) { try { $site = Invoke-RestMethod -Uri ('http://127.0.0.1:' + $port + '/api/site') -TimeoutSec 2; if ($site.name -ne 'Wikist') { throw 'The listener is not Wikist.' } } catch { Write-Error ('Refusing to stop port ' + $port + ': ' + $_.Exception.Message); exit 3 }; Stop-Process -Id $listener.OwningProcess; Start-Sleep -Milliseconds 700 }"
