(function (global) {
  ...
  class CommunityQaController {
    constructor(options) {
      ...
      this.t = (value) => global.WikistI18n?.t ? global.WikistI18n.t(value) : String(value || "");
      this.bootstrap = null;
      this.currentValue = "";
    ...
    communityNav(active = "questions") {
      ...
      return `<nav class="qa-section-tabs" aria-label="${this.escape(this.t("问答社区分区"))}">${items.map(([id, label, href]) => `<a class="${active === id ? "active" : ""}" href="${href}" ${active === id ? 'aria-current="page"' : ""}>${this.escape(this.t(label))}</a>`).join("")}</nav>`;
    }
    ...
    subpagePager(payload, section, params = {}) {
      ...
      if (info.pages <= 1) return `<footer class="qa-pager is-single"><span>${this.t(`共 ${info.total} 条`)}</span></footer>`;
      return `<nav class="qa-pager" aria-label="${this.escape(this.t(`${section}分页`))}"><a class="${info.page <= 1 ? "is-disabled" : ""}" href="${href(Math.max(1, info.page - 1))}">${this.t("上一页")}</a><span>${this.t(`第 ${info.page} / ${info.pages} 页 · 共 ${info.total} 条`)}</span><a class="${info.page >= info.pages ? "is-disabled" : ""}" href="${href(Math.min(info.pages, info.page + 1))}">${this.t("下一页")}</a></nav>`;
    }
    ...
    async renderTags(params) {
      ...
      this.setTitle("社区标签");
      this.renderToc([]);
      this.root.innerHTML = `<section class="community-qa-shell"><header class="article-head qa-page-head"><div><span class="system-kicker">Community Taxonomy</span><h1>社区标签</h1><p class="article-summary">按数学主题浏览问题，并关注需要持续跟进的领域。</p></div></header>${this.communityNav("tags")}${this.serviceStateHtml(bootstrap, payload)}<form class="qa-tag-search" id="qaTagSearch"><input name="q" type="search" value="${this.escape(query)}" placeholder="搜索标签名称" /><button class="qa-search-button" type="submit">${this.icon("search")}<span>搜索</span></button></form><section class="qa-tag-grid">${(payload.items || []).length ? payload.items.map((tag) => `<article><header data-i18n-skip><a href="#/questions?tag=${encodeURIComponent(tag.name)}"><strong>${this.escape(tag.name)}</strong><small>${this.escape(tag.slug)}</small></a>${tag.recommended ? '<span>推荐</span>' : ""}</header><p data-i18n-skip>${this.escape(tag.descriptionMd || "浏览该主题下的社区问题。")}</p><footer><span>${Number(tag.questionCount || 0)} 个问题 · ${Number(tag.followerCount || 0)} 人关注</span>${this.state.user ? `<button class="qa-tag-follow ${tag.followed ? "active" : ""}" type="button" data-qa-tag-follow="${this.escape(tag.slug)}">${tag.followed ? "已关注" : "关注"}</button>` : ""}</footer></article>`).join("") : '<div class="qa-empty-state"><h2>没有匹配的标签</h2><p>发布问题时可以创建新的主题标签。</p></div>'}</section>${this.subpagePager(payload, "tags", { q: query })}</section>`;
    ...
    async renderCollections(params) {
      ...
      this.root.innerHTML = `<section class="community-qa-shell"><header class="article-head qa-page-head"><div><span class="system-kicker">Knowledge Collection</span><h1>${this.t("社区收藏")}</h1><p class="article-summary">${this.t("保存需要继续阅读、验证或整理回词条的问答内容。")}</p></div></header>${this.communityNav("collections")}<section class="qa-collection-list">${(payload.items || []).length ? payload.items.map((item) => `<article><a href="${this.escape(item.url || "#/questions")}" data-i18n-skip><span>${this.escape(this.t(item.type === "answer" ? "回答" : "问题"))}</span><strong>${this.escape(item.title || this.t("社区内容"))}</strong><p>${this.escape(item.summary || "")}</p><small>${this.formatDate(item.collectedAt)}</small></a><button type="button" data-qa-remove-collection data-object-type="${this.escape(item.type)}" data-object-id="${this.escape(item.id)}">${this.t("取消收藏")}</button></article>`).join("") : `<div class="qa-empty-state"><h2>${this.t("还没有社区收藏")}</h2><p>${this.t("在问题或回答下点击“收藏”即可加入这里。")}</p></div>`}</section>${this.subpagePager(payload, "collections")}</section>`;
    ...
    async renderFollows(params) {
      ...
      this.root.innerHTML = `<section class="community-qa-shell"><header class="article-head qa-page-head"><div><span class="system-kicker">Knowledge Watch</span><h1>${this.t("社区关注")}</h1><p class="article-summary">${this.t("持续接收问题更新、回答与标签动态。")}</p></div></header>${this.communityNav("follows")}<section class="qa-collection-list">${(payload.items || []).length ? payload.items.map((item) => `<article class="${item.status === "closed" ? "is-closed" : ""}"><a href="${this.escape(item.url || "#/questions")}" data-i18n-skip><span>${this.escape(this.t(item.type === "tag" ? "标签" : "问题"))}${item.status === "closed" ? ` · ${this.t("已关闭")}` : ""}</span><strong>${this.escape(item.title || this.t("社区内容"))}</strong><p>${this.escape(item.summary || "")}</p><small>${this.formatDate(item.followedAt)}</small></a><button type="button" data-qa-remove-follow data-object-type="${this.escape(item.type)}" data-object-id="${this.escape(item.id)}">${this.t("取消关注")}</button></article>`).join("") : `<div class="qa-empty-state"><h2>${this.t("还没有社区关注")}</h2><p>${this.t("关注问题或标签后，更新会进入你的通知中心。")}</p></div>`}</section>${this.subpagePager(payload, "follows")}</section>`;
    ...
    async renderActivity(params) {
      ...
      this.setTitle("社区动态");
      this.renderToc([]);
      this.root.innerHTML = `<section class="community-qa-shell"><header class="article-head qa-page-head"><div><span class="system-kicker">Activity Stream</span><h1>${this.t("社区动态")}</h1><p class="article-summary">${this.t("查看你有权访问的公共与组织知识活动。")}</p></div></header>${this.communityNav("activity")}<section class="qa-activity-list">${(payload.items || []).length ? payload.items.map((item) => { const metadata = item.metadata || {}; const questionId = metadata.questionId || (item.objectType === "question" ? metadata.publicId : ""); const objectId = metadata.publicId || ""; const href = questionId ? `#/questions/${encodeURIComponent(questionId)}${item.objectType === "answer" && objectId ? `?answer=${encodeURIComponent(objectId)}` : ""}` : ""; const content = `<strong>${this.escape(this.t(labels[item.eventType] || "社区内容更新"))}</strong><small data-i18n-skip>${this.escape(metadata.title || this.t(({ answer: "社区回答", comment: "社区评论", question: "社区问题" })[item.objectType] || "社区内容"))}</small>`; return `<article><span class="qa-activity-marker"></span><div>${this.authorHtml(item.actor || {}, item.createdAt)}${href ? `<a class="qa-activity-object" href="${href}">${content}</a>` : content}</div></article>`; }).join("") : `<div class="qa-empty-state"><h2>${this.t("还没有社区动态")}</h2><p>${this.t("发布问题或回答后，活动会显示在这里。")}</p></div>`}</section>${this.subpagePager(payload, "activity")}</section>`;
    }
    ...
    formatDate(value) {
      ...
      return new Intl.DateTimeFormat(this.state.uiLanguage === "en" ? "en-US" : "zh-CN", { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" }).format(date);
    }
    ...
    questionRow(question, options = {}) {
      ...
      return `<article class="qa-question-row ${accepted ? "is-solved" : ""} ${closed ? "is-closed" : ""}">
        ...
        <div class="qa-question-copy" data-i18n-skip><a href="${href}"><div class="qa-question-title-row"><h2>${this.escape(question.title)}</h2>${closed ? '<span class="qa-closed-label">已关闭</span>' : ""}</div><p>${this.escape(question.summary || "等待补充问题摘要。")}</p></a>${this.questionOriginsHtml(question, true)}<footer><div class="qa-tag-list">${(question.tags || []).slice(0, 6).map((tag) => `<a href="${tagHref(tag)}">${this.escape(tag)}</a>`).join("")}</div><div class="qa-question-footer-identity">${this.authorHtml(question.author, question.updatedAt || question.createdAt)}${this.questionMetricsHtml(question, "is-mobile")}</div></footer></div>
      </article>`;
    }
