...
- **知识导航与安全移动**：分类和主题路径会自动生成轻量目录页；词条内反向链接和正文链接独立按页加载。资深编辑移动词条时会迁移修订、稳定快照、协作数据、译文和关注，并修复相关 Wiki 链接。
...
- **写作组织与社区审阅**：可创建面向学科或语言的写作组织，分配写作者、译者、审阅者与协调者，认领词条任务并以可追溯的社区共识形成稳定版本或发布译文。
- **学术身份与组织论坛**：组织身份自动同步到通行证与公开主页；组织工作区分为 Markdown 首页、任务、论坛和成员四个分页区域，论坛支持主题检索、状态、置顶、关注、收藏、回复分页和词条关联，讨论可自然转化为写作任务与审阅工作。
...
`npm run check` 执行核心 JavaScript 语法检查；`npm run check:knowledge` 会在临时目录验证链接索引、缺失/孤立词条、别名、关注与定向消息；`npm run check:v08` 验证数学元数据、层级分类/主题、安全移动、链接分页和译文审核；`npm run check:v09` 验证审核门控的翻译记忆、术语表、源文变更提示与移动迁移；`npm run check:v10` 验证写作组织、任务认领、讨论、社区审阅共识和越权边界。`npm run check:search` 验证 FTS5 的中英文检索、单词条更新与删除同步。`tools/` 中还保留了 Markdown、评论评分、备份、收藏、数学建模、消息优先级和安全邮件等功能检查脚本。
...
本地调试新版 UI 时，建议临时关闭 `config/site.config.json` 里的 `assets.cdnBase`，让浏览器直接加载本地 `/assets/app.js` 和 `/assets/styles.css`。如果 `assets.cdnBase` 指向 CDN，但 CDN 没有同步当前版本资源，例如 `wikist-core-20260711-78`，浏览器会继续加载旧资源或得到 404，看起来就像本地没有任何变化。

本地确认命令：

```powershell
curl http://127.0.0.1:8899/
curl http://127.0.0.1:8899/assets/app.js?v=wikist-core-20260711-78
curl http://127.0.0.1:8899/assets/styles.css?v=wikist-core-20260711-78
curl http://127.0.0.1:8899/plugins/wikist-cosmic-experience/cosmic.mjs?v=wikist-core-20260711-78
```
...
- 组织首页使用 Markdown 编写公开介绍与协作边界；`协作任务`、`学术论坛`、`成员` 通过横向子导航独立打开并分别分页，手机端保持可横向浏览的子导航。
...
完整角色规则、接口和社区共识边界见 [写作组织与社区审阅](docs/WRITING_COMMONS.md)。
...
浅色主题下，人机验证 SVG 会自动改用浅色底图与深色算式文字；正文代码块、翻译预览和后台 JSON / CSS / JS 代码区域也会切换到浅色可读配色。如果更新后仍看到黑色验证码或代码块白字，请先确认浏览器加载的是 `wikist-core-20260711-78` 或更新版本，并清理 CDN / 浏览器缓存。
...
```bash
curl https://你的域名/assets/app.js?v=wikist-core-20260711-78
curl https://你的域名/plugins/wikist-cosmic-experience/cosmic.mjs?v=wikist-core-20260711-78
```
...
升级到 `2026-07-11` 或更新版本后，核心前端资源版本应变为 `wikist-core-20260711-78`。如果你使用了 CDN、对象存储或浏览器强缓存，更新后建议清理 CDN 缓存，或在浏览器开发者工具 Network 面板确认 `/assets/app.js?v=wikist-core-20260711-78` 和 `/assets/styles.css?v=wikist-core-20260711-78` 已返回新内容。
...
```bash
curl -I -H "Accept-Encoding: br,gzip" https://你的域名/assets/app.js?v=wikist-core-20260711-78
curl -I https://你的CDN域名/wikist/assets/styles.css?v=wikist-core-20260711-78
journalctl -u wikist -f
```
...
如果 CSS / JS 已经很快，但“切换词条、后台切换选项”仍然卡顿，继续看 Network 里的 API 耗时：`/api/pages/...`、`/api/recent`、`/api/admin/...` 如果持续很慢，通常是云服务器 CPU/磁盘 IO、SQLite 文件权限、反向代理超时或跨区网络造成；如果 API 很快但前端仍卡，请确认浏览器拿到的是 `wikist-core-20260711-78` 之后的新版前端资源。
...
- [写作组织与社区审阅](docs/WRITING_COMMONS.md)
...
- **Knowledge navigation and safe moves**: category/topic paths become lightweight directory pages, while backlinks and outgoing links paginate independently inside an article. A privileged article move carries revisions, reviewed snapshots, collaboration records, translations, watches, and repaired wiki links.
...
- `#/community` lists writing organizations, while `#/organization/<slug>` is the organization overview and `#/organization/<slug>?tab=forum` is a dedicated, paginated academic forum with search, categories, state, pinning, sorting, and reply pages.
...
See [Writing Commons And Community Review](docs/WRITING_COMMONS.md) for roles, consensus boundaries, API routes, and verification.
...
```bash
curl https://your-domain/assets/app.js?v=wikist-core-20260711-78
curl https://your-domain/plugins/wikist-cosmic-experience/cosmic.mjs?v=wikist-core-20260711-78
```
...
After updating to the `2026-07-11` build or later, the core frontend asset version should be `wikist-core-20260711-78`. If you use a CDN, object storage, or aggressive browser caching, purge the CDN cache or verify in the browser Network panel that `/assets/app.js?v=wikist-core-20260711-78` and `/assets/styles.css?v=wikist-core-20260711-78` are serving the new files.
...
```bash
curl -I -H "Accept-Encoding: br,gzip" https://your-domain/assets/app.js?v=wikist-core-20260711-78
```
