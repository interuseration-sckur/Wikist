...
本地调试新版 UI 时，建议临时关闭 `config/site.config.json` 里的 `assets.cdnBase`，让浏览器直接加载本地 `/assets/app.js` 和 `/assets/styles.css`。如果 `assets.cdnBase` 指向 CDN，但 CDN 没有同步当前版本资源，例如 `wikist-core-20260712-96`，浏览器会继续加载旧资源或得到 404，看起来就像本地没有任何变化。

本地确认命令：

```powershell
curl http://127.0.0.1:8899/
curl http://127.0.0.1:8899/assets/app.js?v=wikist-core-20260712-96
curl http://127.0.0.1:8899/assets/styles.css?v=wikist-core-20260712-96
curl http://127.0.0.1:8899/plugins/wikist-cosmic-experience/cosmic.mjs?v=wikist-core-20260712-96
```
...
浅色主题下，人机验证 SVG 会自动改用浅色底图与深色算式文字；正文代码块、翻译预览和后台 JSON / CSS / JS 代码区域也会切换到浅色可读配色。如果更新后仍看到黑色验证码或代码块白字，请先确认浏览器加载的是 `wikist-core-20260712-96` 或更新版本，并清理 CDN / 浏览器缓存。
...
```bash
curl https://你的域名/assets/app.js?v=wikist-core-20260712-96
curl https://你的域名/plugins/wikist-cosmic-experience/cosmic.mjs?v=wikist-core-20260712-96
```
...
升级到 `2026-07-11` 或更新版本后，核心前端资源版本应变为 `wikist-core-20260712-96`。如果你使用了 CDN、对象存储或浏览器强缓存，更新后建议清理 CDN 缓存，或在浏览器开发者工具 Network 面板确认 `/assets/app.js?v=wikist-core-20260712-96` 和 `/assets/styles.css?v=wikist-core-20260712-96` 已返回新内容。
...
```bash
curl -I -H "Accept-Encoding: br,gzip" https://你的域名/assets/app.js?v=wikist-core-20260712-96
curl -I https://你的CDN域名/wikist/assets/styles.css?v=wikist-core-20260712-96
journalctl -u wikist -f
```
...
如果 CSS / JS 已经很快，但“切换词条、后台切换选项”仍然卡顿，继续看 Network 里的 API 耗时：`/api/pages/...`、`/api/recent`、`/api/admin/...` 如果持续很慢，通常是云服务器 CPU/磁盘 IO、SQLite 文件权限、反向代理超时或跨区网络造成；如果 API 很快但前端仍卡，请确认浏览器拿到的是 `wikist-core-20260712-96` 之后的新版前端资源。
...
```bash
curl https://your-domain/assets/app.js?v=wikist-core-20260712-96
curl https://your-domain/plugins/wikist-cosmic-experience/cosmic.mjs?v=wikist-core-20260712-96
```
...
After updating to the `2026-07-11` build or later, the core frontend asset version should be `wikist-core-20260712-96`. If you use a CDN, object storage, or aggressive browser caching, purge the CDN cache or verify in the browser Network panel that `/assets/app.js?v=wikist-core-20260712-96` and `/assets/styles.css?v=wikist-core-20260712-96` are serving the new files.
...
```bash
curl -I -H "Accept-Encoding: br,gzip" https://your-domain/assets/app.js?v=wikist-core-20260712-96
```
