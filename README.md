...
本地调试新版 UI 时，建议临时关闭 `config/site.config.json` 里的 `assets.cdnBase`，让浏览器直接加载本地 `/assets/app.js` 和 `/assets/styles.css`。如果 `assets.cdnBase` 指向 CDN，但 CDN 没有同步当前版本资源，例如 `wikist-core-20260712-100`，浏览器会继续加载旧资源或得到 404，看起来就像本地没有任何变化。

本地确认命令：

```powershell
curl http://127.0.0.1:8899/
curl http://127.0.0.1:8899/assets/app.js?v=wikist-core-20260712-100
curl http://127.0.0.1:8899/assets/styles.css?v=wikist-core-20260712-100
curl http://127.0.0.1:8899/plugins/wikist-cosmic-experience/cosmic.mjs?v=wikist-core-20260712-100
```

如果修改了 `config/site.config.json`、服务端代码或资源版本号，重启本地服务后再刷新浏览器。浏览器仍不变时，使用强制刷新或清理站点缓存。

Windows 使用 `--strategy=local` 更新时如果出现 `execFileSync npm.cmd EINVAL`，请升级到 `0.13.0` 或更新版本。新版会直接调用当前 Node.js 安装附带的 `npm-cli.js`，同时继续保护本地配置、SQLite、词条、修订、归档与上传文件。

`0.13.0` 起，`GET /api/pages` 使用不生成正文 HTML 的增量元数据目录；浏览器启动请求 `/api/pages?page=1&limit=200`，同时保留真实词条总数。目录热缓存默认 5 秒，单词条热缓存 1 秒，应用内保存、移动、恢复与删除会立即失效相关缓存。可运行 `npm run check:performance` 查看 120 词条冷目录、热目录和搜索回退自检结果。
...
浅色主题下，人机验证 SVG 会自动改用浅色底图与深色算式文字；正文代码块、翻译预览和后台 JSON / CSS / JS 代码区域也会切换到浅色可读配色。如果更新后仍看到黑色验证码或代码块白字，请先确认浏览器加载的是 `wikist-core-20260712-100` 或更新版本，并清理 CDN / 浏览器缓存。
...
```bash
curl https://你的域名/assets/app.js?v=wikist-core-20260712-100
curl https://你的域名/plugins/wikist-cosmic-experience/cosmic.mjs?v=wikist-core-20260712-100
```
...
安装器不再要求公开 `Origin` 与 Node.js 收到的内部 `Host` 相等，因此可兼容同机 Nginx、Docker、面板反代、隧道和多层代理。安装安全由十分钟短时令牌、客户端限流键绑定、HTTP(S) 来源格式校验和浏览器跨站请求标记共同保证。
...
升级到 `0.13.0` 或更新版本后，核心前端资源版本应变为 `wikist-core-20260712-100`。如果你使用了 CDN、对象存储或浏览器强缓存，更新后建议清理 CDN 缓存，或在浏览器开发者工具 Network 面板确认 `/assets/app.js?v=wikist-core-20260712-100` 和 `/assets/styles.css?v=wikist-core-20260712-100` 已返回新内容。
...
```bash
curl -I -H "Accept-Encoding: br,gzip" https://你的域名/assets/app.js?v=wikist-core-20260712-100
curl -I https://你的CDN域名/wikist/assets/styles.css?v=wikist-core-20260712-100
journalctl -u wikist -f
```
...
如果 CSS / JS 已经很快，但“切换词条、后台切换选项”仍然卡顿，继续看 Network 里的 API 耗时：`/api/pages/...`、`/api/recent`、`/api/admin/...` 如果持续很慢，通常是云服务器 CPU/磁盘 IO、SQLite 文件权限、反向代理超时或跨区网络造成；如果 API 很快但前端仍卡，请确认浏览器拿到的是 `wikist-core-20260712-100` 之后的新版前端资源。
...
### Ubuntu 云端实战排障：安装、更新与 93 词条知识包

以下场景均以 Ubuntu、`/opt/wikist`、systemd 服务 `wikist`、Nginx HTTPS 反向代理，以及已经导入 93 个有限群论词条的站点为背景。先确认实际运行用户和服务状态：

```bash
sudo systemctl show wikist -p User -p Group -p WorkingDirectory
sudo systemctl status wikist --no-pager
sudo journalctl -u wikist -n 120 --no-pager
```

**8899 端口已被占用，直接运行 `node server.js` 失败。**

生产环境不要在 systemd 服务旁再启动第二个进程。检查监听者并重启现有服务：

```bash
sudo ss -ltnp '( sport = :8899 )'
sudo systemctl restart wikist
curl -I http://127.0.0.1:8899/
```

Windows 的 `run-wikist-server.cmd --restart` 不适用于 Ubuntu；云端统一使用 `systemctl`。

**创建词条时报 `EACCES: permission denied`。**

代码可以由 `root` 管理，但运行期目录必须归 systemd 的 Wikist 用户所有。下面假设服务用户和组都是 `wikist`：

```bash
cd /opt/wikist
sudo install -d -o wikist -g wikist config data content content/pages content/revisions content/deleted public/uploads plugins/vendor
sudo chown -R wikist:wikist config data content public/uploads plugins/vendor
sudo find config data content public/uploads plugins/vendor -type d -exec chmod 750 {} \;
sudo find config data content public/uploads plugins/vendor -type f -exec chmod 640 {} \;
sudo systemctl restart wikist
```

不要对整个站点使用 `chmod -R 777`。如果服务实际使用 `www-data` 或其他账号，把命令里的 `wikist:wikist` 换成 `systemctl show` 返回的用户和组。

**后台拉取插件或创建插件时报 `mkdir EACCES`。**

只拉取上游源码时，让服务用户拥有 `plugins/vendor/` 即可。若还要从后台创建新的插件目录，服务必须能写 `plugins/`：

```bash
sudo chown -R wikist:wikist /opt/wikist/plugins/vendor
# 仅在确实启用“后台创建插件”时执行下一行
sudo chown wikist:wikist /opt/wikist/plugins
sudo systemctl restart wikist
```

插件目录中的代码仍受可信边界约束：第三方服务端模块默认不执行，拉取后应先审阅清单、权限和 Hook 声明。

**反代域名访问安装器时提示“安装请求来源不受信任”。**

升级到 `0.12.7` 或更新版本。新版不再要求浏览器公网 `Origin` 与内部反代 `Host` 完全相同，而是使用十分钟一次性安装令牌、HTTP(S) 来源校验、客户端键绑定和浏览器跨站声明检查。Nginx 仍应传递标准头：
...
更新并重启后重新打开 `/install.html`，不要复用升级前已打开的安装页标签。

**自己触发了请求保护，页面显示数百秒后重试。**

当前单机限流桶保存在进程内，管理员可通过重启立即清空：

```bash
sudo systemctl restart wikist
```

默认安装策略已经调整为十分钟 60 次、触发后 60 秒解锁。站点上线后可在 `config/site.config.json` 的 `security.firewall` 下调整 `general`、`api`、`write`、`auth` 和 `install`，但不要关闭认证与写入保护。只有 Nginx 是受控的直接上游时才启用 `trustedProxy`，否则伪造转发头会污染客户端识别。

**使用更新脚本升级。**

先保留运行期数据，再由脚本拉取核心代码、执行检查并重启服务：
...
更新器保护 `config/site.config.json`、SQLite、词条、修订、归档、上传和第三方源码缓存。若检查失败，读取 `data/updates/update-*.json` 和 `journalctl`，不要直接删除数据库或内容目录。

**93 个导入词条中的 `$...$` 行内公式全部未渲染。**

这是 `0.12.7` 及更早发布包的服务端 Markdown 顺序问题，不是词条包损坏，也无需重新导入。旧渲染器会先把 `$S_3$`、`$F^*(G)$` 中的下划线和星号当作 Markdown 强调。`0.12.8` 起会先保护 TeX，再处理强调，并统一支持：
...
如果页面源代码已经出现 `<div class="math-block">\\[...\\]</div>`，但仍显示原始 TeX，说明浏览器还在使用旧 MathJax 配置。确认页面加载的是 `app.js?v=wikist-core-20260712-100`，强制刷新浏览器，并清理 CDN 的 `/assets/app.js` 与 HTML 缓存。

**导入或撤回有限群论知识包。**

在知识包解压目录中先预演，再确认写入；作者名应替换为站点中的实际账号：

```bash
node import-bundle.cjs --root=/opt/wikist --author=admin
node import-bundle.cjs --root=/opt/wikist --author=admin --yes
```

旧版错误知识包应使用随包提供并匹配该版本清单的回滚脚本，先预演再加 `--yes`。回滚工具只移除清单中且内容指纹匹配的旧导入，避免误删后来人工修改的词条。任何批量导入或回滚前都应先从后台生成并校验整站备份。
...
```bash
curl https://your-domain/assets/app.js?v=wikist-core-20260712-100
curl https://your-domain/plugins/wikist-cosmic-experience/cosmic.mjs?v=wikist-core-20260712-100
```
...
The installer no longer requires the public `Origin` to equal the internal `Host` seen by Node.js, so same-host Nginx, Docker, control-panel proxies, tunnels, and multi-hop proxies work without false rejection. A short-lived challenge token, rate-limit client binding, HTTP(S) origin parsing, and browser cross-site metadata retain installation CSRF protection.
...
After updating to `0.13.0` or later, the core frontend asset version should be `wikist-core-20260712-100`. If you use a CDN, object storage, or aggressive browser caching, purge the CDN cache or verify in the browser Network panel that `/assets/app.js?v=wikist-core-20260712-100` and `/assets/styles.css?v=wikist-core-20260712-100` are serving the new files.
...
```bash
curl -I -H "Accept-Encoding: br,gzip" https://your-domain/assets/app.js?v=wikist-core-20260712-100
```
