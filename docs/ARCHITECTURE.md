...
The v0.10 collaboration commons adds organizations, memberships, tasks, discussion posts/replies, votes, and consensus records to Passport SQLite. These are collaboration metadata rather than an alternate article store. Consensus is scoped to one organization and one immutable page or translation snapshot; threshold approval reuses the existing stable-revision or published-translation path. See [Collaboration Commons And Community Review](WRITING_COMMONS.md).
